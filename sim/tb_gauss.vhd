--------------------------------------------------------------------------------
-- testbench for the gaussian filter
--
-- Checks performed:
...
architecture sim of tb_gauss is
  -- ------------------------------------------------------------------
  ...
  -- ------------------------------------------------------------------
  -- Helper: 3x3 convolution with [1 2 1; 2 4 2; 1 2 1] / 16 (reference model)
  -- Window order is row major, w(4) is the centre.
  ...
  begin
    s := w(0) + 2*w(1) + w(2)
         + 2*w(3) + 4*w(4) + 2*w(5)
         + w(6) + 2*w(7) + w(8);
    return (s + 8) / 16; -- s + 8 is never negative -> (s+8) >> 4
  end function;
...
begin
  ...
  -- ------------------------------------------------------------------
  -- Generate test image + reference (fully deterministic, no randomness)
  gen_img : process
    variable img : img_t;
    variable ref : img_t;
    variable win : win9_t;
  ...
  begin
    -- base pattern: dark / bright blocks with a gradient inside each block
    for y in 0 to H-1 loop
      for x in 0 to W-1 loop
        if ((x/4) + (y/4)) mod 2 = 0 then
          img(y,x) := 20 + (x*3 + y*5) mod 60;
        else
          img(y,x) := 200 + (x*7 + y*11) mod 55;
        end if;
      end loop;
    end loop;

    -- explicit extremes: a fully black and a fully white line check that
    -- rounding neither underflows (0 -> 0) nor overflows (255 -> 255)
    for x in 0 to W-1 loop
      img(2,x) := 0;
      img(3,x) := 255;
    end loop;

    -- impulse inside a flat black area: the response reproduces the kernel weights directly, 
    -- so a mixed up weight or a shifted window shows up immediately
    for y in 5 to H-1 loop
      for x in 5 to 11 loop
        img(y,x) := 0;
      end loop;
    end loop;
    img(6,8) := 255;

    -- reference image: 3x3 gauss, clamped borders on all four sides
    for y in 0 to H-1 loop
      for x in 0 to W-1 loop
        idx := 0;
  ...
  begin
    ...
    for y in 0 to H-1 loop
      for x in 0 to W-1 loop
        ...
        -- occasional gap in the source stream, driving X to prove that the DUT latches nothing without fire
        next_rnd(seed, r);
